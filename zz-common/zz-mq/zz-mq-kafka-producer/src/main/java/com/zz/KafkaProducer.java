    ...
    public <T> void  sendMsg(SendData<T> sendData) {
        try {
            ...
            if(ObjectUtils.isEmpty(sendData.getData())){
                ...
                log.info(sendData.getTopic()+"发送数据"+JSON.toJSONString(sendData.getData()));
            }
            //发送消息
            ListenableFuture<SendResult<String, Object>> future = kafkaTemplate.send(sendData.getTopic(), JSON.toJSONString(sendData.getData()));
